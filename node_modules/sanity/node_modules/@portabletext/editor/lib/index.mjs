...
function createArrayedPath(point, editor) {
  ...
  const childPath = [point.path[2]], childIndex = block.children.findIndex((child) => isEqual([{
    _key: child._key
  }], childPath));
  ...
  $[16] !== editor || $[17] !== element ? (t9 = (event_0) => {
    ...
    if (targetBlock) {
      ...
      if (dragPosition === "bottom" && !isBefore) {
        ...
        targetPath = Path.next(targetPath), debug$l(`Adjusting targetPath from ${JSON.stringify(originalPath)} to ${JSON.stringify(targetPath)}`);
      }
      if (dragPosition === "top" && isBefore && targetPath[0] !== editor.children.length - 1) {
        const originalPath_0 = targetPath;
        targetPath = Path.previous(targetPath), debug$l(`Adjusting targetPath from ${JSON.stringify(originalPath_0)} to ${JSON.stringify(targetPath)}`);
      }
      if (Path.equals(targetPath, myPath)) {
        event_0.preventDefault(), debug$l("targetPath and myPath is the same, not moving");
        return;
      }
      debug$l(`Moving element ${element._key} from path ${JSON.stringify(myPath)} to ${JSON.stringify(targetPath)} (${dragPosition})`), Transforms.moveNodes(editor, {
        at: myPath,
        to: targetPath
      }), editor.onChange();
      return;
      ...
      if (undos.length > 0) {
        ...
        if (debug$k("Undoing", step), step.operations.length > 0) {
          ...
          try {
            ...
            debug$k("Could not perform undo step", err), remotePatches.splice(0, remotePatches.length), Transforms.deselect(editor), editor.history = {
              undos: [],
              redos: []
      ...
      if (redos.length > 0) {
        ...
        if (debug$k("Redoing", step), step.operations.length > 0) {
          ...
          try {
            ...
            debug$k("Could not perform redo step", err), remotePatches.splice(0, remotePatches.length), Transforms.deselect(editor), editor.history = {
              undos: [],
              redos: []
...
function useSyncValue(props) {
  ...
  return useMemo(() => {
    const updateFunction = (value) => {
      ...
      if ((!value || value.length === 0) && (debug$j("Value is empty"), Editor.withoutNormalizing(slateEditor, () => {
              ...
              withoutPatching(slateEditor, () => {
                ...
                slateValueFromProps.forEach((currentBlock, currentBlockIndex) => {
                  ...
                  if (!oldBlock && isValid) {
                    ...
                    debug$j.enabled && debug$j("Validating and inserting new block in the end of the value", currentBlock), validation_0.valid || validation_0.resolution?.autoResolve ? Transforms.insertNodes(slateEditor, currentBlock, {
...
function _updateBlock(slateEditor, currentBlock, oldBlock, currentBlockIndex) {
  if (Transforms.setNodes(slateEditor, currentBlock, {
    ...
    currentBlock.children.length < oldBlockChildrenLength && Array.from(Array(oldBlockChildrenLength - currentBlock.children.length)).forEach((_, index) => {
      ...
      childIndex > 0 && (debug$j("Removing child"), Transforms.removeNodes(slateEditor, {
      ...
      if (isChildChanged)
        if (currentBlockChild._key === oldBlockChild?._key) {
          debug$j("Updating changed child", currentBlockChild, oldBlockChild), Transforms.setNodes(slateEditor, currentBlockChild, {
            at: path
          });
          ...
          isSpanNode && isTextChanged ? (Transforms.delete(slateEditor, {
            ...
            at: path
          }), slateEditor.onChange()) : isSpanNode || (debug$j("Updating changed inline object child", currentBlockChild), Transforms.setNodes(slateEditor, {
            _key: VOID_CHILD_KEY
          }, {
            at: [...path, 0],
            voids: !0
          }));
        } else oldBlockChild ? (debug$j("Replacing child", currentBlockChild), Transforms.removeNodes(slateEditor, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slateEditor.onChange()) : oldBlockChild || (debug$j("Inserting new child", currentBlockChild), Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slateEditor.onChange());
    });
  }
...
function createWithPortableTextBlockStyle(editorActor, types) {
  ...
  return function(editor) {
    ...
    }, editor.pteHasBlockStyle = (style) => editor.selection ? [...Editor.nodes(editor, {
      at: editor.selection,
    ...
    return editor.pteWithHotKeys = (event) => {
      ...
      if ((isTab || isShiftTab) && editor.selection) {
        const [focusChild] = Editor.node(editor, editor.selection.focus, {
          depth: 2
        }), [focusBlock] = isPortableTextSpan$1(focusChild) ? Editor.node(editor, editor.selection.focus, {
          depth: 1
        }) : [], hasAnnotationFocus = focusChild && isPortableTextTextBlock(focusBlock) && isPortableTextSpan$1(focusChild) && (focusChild.marks || []).filter((m) => (focusBlock.markDefs || []).map((def) => def._key).includes(m)).length > 0, [start] = Range.edges(editor.selection), atStartOfNode = Editor.isStart(editor, start, start.path);
